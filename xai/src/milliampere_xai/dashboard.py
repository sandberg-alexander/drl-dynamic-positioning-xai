...
class NedRender(VesselRender):
    ...
    def render(self, x_ned_err, y_ned_err, psi_err, target_pose):
        ...
        # error = target - vessel → vessel world pos = -error (relative to target at origin)
...
class ShapExplainRender(VesselRender, Utilities):
    ...
    def _find_explanation2(
        ...
        _dists_tot = np.hypot(base_scaled[:, 0, :], base_scaled[:, 1, :])  # noqa: F841
    ...
    def _find_explanation(self, lists, n=3, list_in_list=True):
        combined_values = []
        if list_in_list:
            ...
            for i in range(len(lists[0])):
                ...
                total = sum(
                    np.sqrt(lists[2 * j][i] ** 2 + lists[2 * j + 1][i] ** 2)
    ...
    def _calculate_action_direction(self, shap_values, thrusters, overide=None):
        ...
        for i in range(2):
            ...
            moment_norm = self._calculate_total_moment(vectors)
            s = np.append(s, moment_norm)
            x_or_y[i] = np.argmax(np.abs(s))

            value[i] = s[x_or_y[i]]
            if i == 1 and overide is not None:
                value[i] = s[overide]
                a = s
    ...
    def _calculate_ad(self, thrusters):
        ...
        moment_norm = self._calculate_total_moment(thrusters)
        s = np.append(s, moment_norm)
        return s

    def _calculate_total_moment(self, thrusters):
        total_moment_prime = 0.0
        for i, ((x, y), (n, alpha)) in enumerate(zip(self.actuator_pos, thrusters)):
            rad = np.deg2rad(90 * (i + 1) - alpha)
            if x * y < 0:
    ...
    def _draw_explanation(
        ...
        if self.idx == 0:
            self._draw_distance(
                ...
                self._scalar2pygame(error_x),
                self._degrees2pygame(0),
                Color.RED.value,
                label=f"{error_x:.2f} m",
            )  # Explaination
            ...
            i, idx_value, j, total_value, x, y, _ = self._calculate_action_direction(
                shap_values_RPM, thrusters, overide=0
            )
            ...
            if i == 0:
                ...
                self._draw_arrow(
                    ...
                    Color.DESIRED_LIGHT_YELLOW.value,
                    (self.center[0] + 10, self.center[1]),
                )  # SHAP RPM in y direction
            elif i == 2:
                self._draw_curved_arrow(
                    np.deg2rad(idx_value),
                    self.center,
                    Color.DESIRED_LIGHT_YELLOW.value,
                    radius=30,
                )  # SHAP RPM in moment direction
            else:
                ...
            ...
            self._draw_distance(
                ...
                (
                    self.center[0],
                    self.center[1]
                    + self._scalar2pygame(self.VESSEL_LENGTH / 2)
                    + self.explain_offset,
                ),
                self._scalar2pygame(error_y),
                self._degrees2pygame(90),
                Color.RED.value,
                label=f"{error_y:.2f} m",
            )  # Explaination
            _, total_rpm, _ = self._calculate_ad(thrusters)
            total_rpm *= self.SCALE

            i, idx_value, j, total_value, _, _, _ = self._calculate_action_direction(
                shap_values_RPM, thrusters, overide=1
            )
            # print(f"i:{i}, idx_value{idx_value}\nj:{j}, total_value:{total_value}")
            ...
            self._draw_angle(
                self.surface,
                self.center,
                np.deg2rad(error_psi),
                Color.RED.value,
                radius=self._scalar2pygame(self.VESSEL_LENGTH / 2)
                + self.explain_offset,
                label=f"{error_psi:.0f} °",
            )
            ...
            i, idx_value, j, total_value, _, _, _ = self._calculate_action_direction(
                shap_values_RPM, thrusters, overide=2
            )
            # print(f"i:{i}, idx_value{idx_value}\nj:{j}, total_value:{total_value}")
            ...
            self._draw_curved_arrow(
                np.deg2rad(total_value), self.center, Color.PURPLE.value, radius=42
            )
            self._draw_curved_arrow(
                np.deg2rad(moment), self.center, Color.VELOCITY_GREEN.value, radius=40
            )
            ...
            total_rpm_x *= self.SCALE
            total_rpm_y *= self.SCALE
            magnitude = np.sqrt(total_rpm_x**2 + total_rpm_y**2)  # noqa: F841
            angle_radians = np.arctan2(total_rpm_y, total_rpm_x)
            angle_degrees = np.degrees(angle_radians)  # noqa: F841
            ...
            self._draw_curved_arrow(
                total_value, self.center, Color.PURPLE.value, radius=radius + 2
            )
            self._draw_curved_arrow(
                moment, self.center, Color.VELOCITY_GREEN.value, radius=radius
            )
            ...
            if i == 0:
                ...
                self._draw_curved_arrow(
                    idx_value, self.center, Color.DESIRED_LIGHT_YELLOW.value, radius=30
                )  # SHAP RPM in moment direction
            else:
                ...
    ...
    def _draw_explanation2(
        ...
        main_feature_vector = self._add_vectors_polar(  # noqa: F841
            [main_feature_vector_base[:2], (-base_vector[0], base_vector[1])]
        )
        tot_vector_base = self._add_vectors_polar(base_vectors, shap_tot_vectors)
        tot_vector = self._add_vectors_polar(  # noqa: F841
            [tot_vector_base[:2], (-base_vector[0], base_vector[1])]
        )
        _, _, moment = self._calculate_ad(thrusters)
        ...
        if self.explain_vector:
            ...
            self._draw_curved_arrow(
                self._scalar2pygame(tot_vector_base[2]) / (1200 * radius),
                self.center,
                Color.PURPLE.value,
                radius=radius + 10,
            )
            # Total angular RPM
            self._draw_curved_arrow(
                self._scalar2pygame(moment) / radius,
                self.center,
                Color.VELOCITY_GREEN.value,
                radius=radius + 10,
            )
        self._draw_curved_arrow(
            self._scalar2pygame(main_feature_vector_base[2]) / (1200 * radius),
            self.center,
            Color.DESIRED_LIGHT_YELLOW.value,
            radius=radius,
            arrowhead_length=5,
        ...
        if self.explain_vector:
            ...
            self._draw_arrow(
                self._degrees2pygame(tot_vector_base[1]),
                self._scalar2pygame(tot_vector_base[0] / 1200),
                Color.PURPLE.value,
                self.center,
            )
        ...
        self._draw_arrow(
            self._degrees2pygame(main_feature_vector_base[1]),
            self._scalar2pygame(main_feature_vector_base[0] / 1200),
            Color.DESIRED_LIGHT_YELLOW.value,
            self.center,
            arrowhead_width=5,
            arrowhead_length=5,
        ...
        error = np.array([n_d * 1200, alpha_d, moment * 1200]) - np.array(
            [tot_vector_base[0], tot_vector_base[1], tot_vector_base[2]]
        )
        ...
        text_surface1 = self.window_font.render(
            f"Accuracy RPM: {acc_RPM:.2f}", self.antialias, acc_RPM_color
        )
        text_rect1 = text_surface1.get_rect()
        text_rect1.topleft = (self._title_offset, self._title_offset * 10)

        text_surface2 = self.window_font.render(
            f"Accuracy angle: {acc_angle:.2f}", self.antialias, acc_angle_color
        )
        ...
        text_surface3 = self.window_font.render(
            f"Accuracy moment: {acc_moment:.2f}", self.antialias, acc_moment_color
        )
    ...
    def _calculate_actuator_inputs(self, action, prev_angle):
        ...
        for i in range(4):
            ...
            thrusters[i] = thrust * MAX_THRUSTER_RPM
    ...
    def _combine_actuator_ref(self, actuator_ref, actuator_pos):
        ...
        for i, ((thrust, angle), (x, y)) in enumerate(zip(actuator_ref, actuator_pos)):
            ...
            if x * y < 0:
                ang_thrust = abs(x) * thrust * np.cos(rad) + abs(y) * thrust * np.sin(
                    rad
                )
            else:
                ang_thrust = abs(x) * thrust * np.sin(rad) + abs(y) * thrust * np.cos(
                    rad
                )
    ...
    def _draw_explanation4(
        ...
        reconstructed_pred_total = np.clip(  # noqa: F841
            summed_array + base_vectors, action_low, action_high
        )  # Clip here
        ...
        vector2 = self._combine_actuator_ref(vec2, self.actuator_pos)  # noqa: F841
        # print('vector2')
        # print(vector2)
        # print('actuator_ref')
        ...
        thrust = np.sqrt(x**2 + y**2)  # noqa: F841
        angle = np.arctan2(y, x)  # noqa: F841
        ang_thrust = self._calculate_angular_thrust(shap_feature_RPM)  # noqa: F841
        # print(ang_thrust)
        ...
        # self._draw_curved_arrow(np.clip(self._scalar2pygame(ang_thrust)/(900*radius),-2*np.pi,2*np.pi), self.center, Color.DESIRED_LIGHT_YELLOW.value, radius=radius, arrowhead_length=5, arrowhead_width=5)
        ...
        # self._draw_curved_arrow(np.clip(self._scalar2pygame(vector[2])/(900*radius),-2*np.pi,2*np.pi), self.center, Color.DESIRED_LIGHT_YELLOW.value, radius=radius, arrowhead_length=5, arrowhead_width=5)
    ...
    def render(
        ...
        # print(1)
        # #self._find_explanation(shap_values_EV, n=1, list_in_list=False)
        ...
        # self._find_explanation(shap_values_action, n=1, list_in_list=self.RPM_true)
        # print(2)
        # self.idx=1
        ...
        self.draw_vessel()
        # print(5)
        # self._draw_explanation(shap_values_EV, shap_values_RPM, thrusters, y_err, x_err, psi_err, u_hat, v_hat, r_hat)
        # self._draw_explanation2(y_err, x_err, psi_err, u_hat, v_hat, r_hat, shap_values_RPM, shap_values_angles, base_vectors, n_d, alpha_d, thrusters)
...
class ShapRender(Window):
    ...
    def _find_top_n_abs_combined_indices(
        self, lists, base_value, action_low, action_high, n=3, list_in_list=True
    ):
        combined_values = []
        if list_in_list:
            # Calculate the combined absolute values for each index
                ...
                total = sum(
                    np.sqrt(
                        ...
                        ** 2
                        + (
                            np.clip(
                                lists[2 * j + 1][i] + base_value[2 * j + 1],
                                action_low[2 * j + 1],
                                action_high[2 * j + 1],
                            )
                        )
    ...
    def _draw_bars(
        self, shap_values, shap_values3, base_value, action_low, action_high
    ):
        """
        Draw four horizontal bars:
        – the three most influential individual features
        ...
        top_idx = self._find_top_n_abs_combined_indices(
            data, base_value, action_low, action_high, list_in_list=self.RPM_true, n=3
        )
        # print(f"top_idx:{top_idx}")

        others_idx = [
            i for i in range(len(self.feature_names_full)) if i not in top_idx
        ]
        ...
        for bar_no, feat_idx in enumerate(draw_order):
            ...
            for seg in range(self.num_bar_seg):
                if feat_idx == -1:  # -------- OTHERS ------------
                    ...
                    for jj in others_idx:
                        reco = np.clip(
                            np.array(data)[:, jj] + base_value, action_low, action_high
                        )
                        seg_val += np.clip(
                    ...
                    reco = np.clip(
                        np.array(data)[:, feat_idx] + base_value,
                        action_low,
                        action_high,
                    )
                    seg_val = np.clip(np.hypot(reco[2 * seg], reco[2 * seg + 1]), 0, 1)
