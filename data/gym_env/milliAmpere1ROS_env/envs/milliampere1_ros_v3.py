...
class MilliAmpere1RosEnvV3(gym.Env):
    ...
    def _get_reward(self):
        ...
        for i, angle in enumerate(self.actual_angles):
            min_angle, max_angle = self.actuator_constraints[i]
            min_minus_45 = self._ssa_rad(min_angle - np.pi/4)
            max_plus_45 = self._ssa_rad(max_angle + np.pi/4)
            ...
            elif max_plus_45 < angle < self._ssa_rad(max_angle)+np.pi/2:
                self.R_constraint += -0.1 - self.w_constraint * abs(angle - (self._ssa_rad(max_angle)+np.pi/2))

            # CASE 5: Thruster angle is within 45° more than max_angle
            elif self._ssa_rad(max_angle) < angle <= max_plus_45:
                self.R_constraint += -0.1 - self.w_constraint * abs(angle - self._ssa_rad(max_angle))
    ...
    def _calculate_actuator_inputs(self, action):
        ...
        for i in range(4):
            ...
            # CASE 2: Thruster angle is in opposite quadrant
                ...
                angle = self._ssa_rad(angle + np.pi)
                thrust = -thrust
