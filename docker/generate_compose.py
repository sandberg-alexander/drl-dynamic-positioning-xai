...
def _gpu_config():
    """Return environment, devices, and deploy config for NVIDIA GPU passthrough."""
    env = {
    ...
    return env, devices, deploy


def generate_parallel_compose(n_envs, gpu=False):
    ...
    # --- GPU passthrough ---
    if gpu:
        gpu_env, gpu_devices, gpu_deploy = _gpu_config()
        for svc_name in ("drl", "xai"):
            services[svc_name]["environment"].update(gpu_env)
            services[svc_name]["devices"] = gpu_devices
            services[svc_name]["deploy"] = gpu_deploy
    ...
    try:
        with open(PARALLEL_OUTPUT_FILE, "w") as f:
            f.write(
                f"# Auto-generated parallel training config ({n_envs} simulators{', GPU' if gpu else ''}).\n"
                f"# Generated by: python3 generate_compose.py parallel --n-envs {n_envs}{'--gpu' if gpu else ''}\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gpu",
        action="store_true",
        help="Enable NVIDIA GPU passthrough on drl/xai (for 'parallel' mode).",
    )
    ...
    if args.mode == "parallel":
        ...
        generate_parallel_compose(args.n_envs, gpu=args.gpu)
    else:
