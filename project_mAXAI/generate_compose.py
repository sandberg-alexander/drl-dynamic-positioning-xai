...
def get_local_ip_suggestion():
    ...
    try:
        ...
        if ip.startswith(('192.168.', '10.', '172.')) or ':' in ip: # Include IPv6 check loosely
             return ip
        else:
             print(f"Warning: Auto-detected IP {ip} might not be a typical LAN IP.", file=sys.stderr)
             return ip
...
def generate_laptop_compose(mode, remote_ip=None, local_ip=None):
    ...
    """
    print(f"--- Generating Laptop Configuration ---")
    print(f"Mode: {mode}")
    ...
    if mode == 'local':
        ...
        print(f"Local mode selected.")
        print(f"ROS_IP will be unset (using Docker bridge network DNS).")
        ...
        if not local_ip:
            ...
            print(f"Auto-detected Laptop IP: {local_ip} (Use --local-ip to override if incorrect)")
        if local_ip == "<your_laptop_ip>":
             print("Error: Could not auto-detect local IP. Please provide it using --local-ip.", file=sys.stderr)
             sys.exit(1)
        ...
        use_docker_network = False # Communication via host network (network_mode: host)
        dependency_target = ""
        print(f"Remote mode selected.")
        print(f"Remote PC (Master) IP: {remote_ip}")
        print(f"Laptop IP (for ROS_IP): {laptop_ip_to_set}")
        print(f"ROS Master URI: {ros_master_uri}")
    else:
        print(f"Error: Invalid mode '{mode}'. Use 'local' or 'remote'.", file=sys.stderr)
        sys.exit(1)
    ...
    modified_content = modified_content.replace(DEPENDENCY_PLACEHOLDER, dependency_target)

    # Replace or remove ROS_IP line based on mode
    if laptop_ip_to_set: # Remote mode: Replace placeholder with Laptop's IP
        modified_content = modified_content.replace(ROS_IP_PLACEHOLDER, laptop_ip_to_set)
        ...
        print(f"Removing ROS_IP lines for local mode.")


    # Add/Remove Network Blocks
    if use_docker_network:
        modified_content = modified_content.replace(NETWORK_PLACEHOLDER, LOCAL_NETWORK_BLOCK)
        modified_content = modified_content.replace(NETWORK_DEFINITION_PLACEHOLDER, LOCAL_NETWORK_DEFINITION)
    else:
        modified_content = re.sub(rf'^\s*{NETWORK_PLACEHOLDER}.*$\n?', '', modified_content, flags=re.MULTILINE)
        modified_content = re.sub(rf'^\s*{NETWORK_DEFINITION_PLACEHOLDER}.*$\n?', '', modified_content, flags=re.MULTILINE)

    # --- Load YAML to manipulate structure ---
    try:
        data = yaml.safe_load(modified_content)
        if not isinstance(data, dict) or 'services' not in data:
             print(f"Error: Invalid YAML structure after replacements.", file=sys.stderr)
             sys.exit(1)
    ...
    if mode == 'local':
        print("Local mode detected. Removing 'network_mode: host' from drl and xai services.")
        for service_name in ['drl', 'xai']:
            if service_name in services_dict and 'network_mode' in services_dict[service_name]:
        ...
        if LOCAL_SIM_SERVICE_NAME in services_dict and 'network_mode' in services_dict[LOCAL_SIM_SERVICE_NAME]:
             print(f"Warning: Found unexpected network_mode in {LOCAL_SIM_SERVICE_NAME} for local mode. Removing.")
    ...
    # Remove networks definitions from services if in remote mode (host mode makes them irrelevant)
         ...
         print("Remote mode detected. Removing 'networks:' definition from services if present.")
         for service_name in ['drl', 'xai']:
              if service_name in services_dict and 'networks' in services_dict[service_name]:
    ...
    if not include_local_simulator:
        if LOCAL_SIM_SERVICE_NAME in services_dict:
            ...
            print(f"Warning: Local service '{LOCAL_SIM_SERVICE_NAME}' not found in parsed data.", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--remote-ip',
        help="IP address of the remote PC (required for 'remote' mode)."
    )
    parser.add_argument(
        '--local-ip',
