...
class VesselRender(Window):
    ...
    def draw_vessel(self, x_err=1, y_err=-1, psi_err=-45, ned=False):
        if ned:
    ...
    def draw_target(self, x_err=1, y_err=-1, psi_err=-45, ned=False):
        if ned:
            shape = self.shape
            circle = self.circle_point
            triangle = self.triangle_shape
...
class BodyRender(VesselRender):
    ...
    def _draw_force_and_moment(self, n_d, alpha_d, moment_d):
    ...
    def render(self, thrusters, n_d, alpha_d, moment_d, u_hat, v_hat, r_hat):
        # init
        self.render_surface()
        ...
        self._draw_force_and_moment(n_d, alpha_d, moment_d)
...
class NedRender(VesselRender):
    ...
    def render(self):
        ...
        # NED render
        self.surface.fill(Color.OCEAN_BLUE.value)
        self.draw_target(ned=True)
        self.draw_vessel(ned=True)
...
class RenderExplaination():
    ...
    def render_frame(self, shap_values, thrusters, n_d, alpha_d, moment_d, u_hat, v_hat, r_hat):
        self._screen.fill(Color.SCREEN_COLOR.value)

        self._body_window.render(thrusters, n_d, alpha_d, moment_d, u_hat, v_hat, r_hat)
        self._ned_window.render()
        self._shap_window_top.render(shap_values)
        self._shap_window_bottom.render(shap_values)
